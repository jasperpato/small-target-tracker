...
def region_growing(gray, binary):
  '''
  Implementation a new region growing function that is applied to centroids 
...
if __name__ == '__main__':
  ...
  grown = region_growing(ctr_gray, b)
  _, ax2 = plt.subplots()
  ax2.set_title("Candidate Match Discrimination")
  ax2.imshow(grown, cmap='gray')

  if sys.argv[1] == '--boxes':
    for box in frames[1][2]:
      ax1.add_patch(patches.Rectangle((box[0] - 0.5, box[1] - 0.5), 
                                      box[2], box[3], 
